...
apiRoute.get("/route", async (request, response) => {
  const result: QuerySnapshot = await firestore.collection(COLLECTION_ROUTES).get()
  const routes: Route[] = []
  result.forEach((doc) => {
    routes.push({
      id: doc.id,
...
apiRoute.get("/route/:id", async (request, response) => {
  ...
  const result: DocumentSnapshot = await firestore.collection(COLLECTION_ROUTES).doc(routeId).get()
  const route: Route = {
    id: result.id,
...
apiRoute.put("/route", async (request, response) => {
  ...
  const result: DocumentReference = await firestore.collection(COLLECTION_ROUTES).add(route)
...
apiRoute.put("/route/:id", async (request, response) => {
  ...
  await firestore.collection(COLLECTION_ROUTES).doc(routeId).update(route)
  response.status(200).send(route)
})

apiRoute.delete("/route/:id", async (request, response) => {
  const routeId: string = request.params.id
  await firestore.collection(COLLECTION_ROUTES).doc(routeId).delete()
